...
#5
select * FROM asignatura inner join curso on curso.id = asignatura.id_curso where curso.grado_id = '7' and curso.cuatrimestre = '1';

#6
select persona.apellido1, persona.apellido2, persona.nombre, departamento.nombre as departamento from persona inner join profesor on profesor.id_profesor = persona.id inner join departamento on profesor.id_departamento = id_departamento order by apellido1 desc, apellido2 desc, nombre desc;

#7
select asignatura.nombre, curso_escolar.anyo_inicio, curso_escolar.anyo_fin, persona.nombre 
from asignatura inner join curso_escolar on curso_escolar.id = asignatura.id 
inner join persona on persona.id = asignatura.id WHERE persona.NIF = '26902806M';

#8
select departamento.nombre where nombre like 'Ingeniería Informática'from departamento inner join profesor on profesor.id = departamento.id;

#9
select * , curso_escolar.anyo_inicio from persona inner join curso_escolar on persona.id = curso_escolar.id where anyo_inicio >=2014 and anyo_inicio <=2015;

#6 siguientes consultas utilizando las cláusulas LEFT JOIN y RIGHT JOIN:

#1
#nombre del departamento, primer apellido, segundo apellido y nombre del profesor/a. 
select departamento.nombre, persona.apellido1, persona.apellido2, persona.nombre from persona left join departamento on persona.id = departamento.id;

#2 Devuelve un listado con los profesores/as que no están asociados a un departamento.
select departamento.nombre, persona.apellido1, persona.apellido2, persona.nombre from persona left join departamento on persona.id = departamento.id where departamento.nombre is null;
...
select *, profesor.id_profesor, persona.apellido1, persona.apellido2, persona.nombre from asignatura 
right join profesor on asignatura.id_profesor = profesor.id_profesor 
left join persona on profesor.id_profesor = persona.id where asignatura.nombre is null;
...
select * from departamento left join profesor on departamento.id = profesor.id_departamento left join asignatura on profesor.id_profesor = asignatura.id_profesor left join alumno_se_matricula_asignatura on asignatura.id = alumno_se_matricula_asignatura.id_asignatura left join curso_escolar on alumno_se_matricula_asignatura.id_curso_escolar = id_curso_escolar where curso_escolar.anyo_inicio is null;

#Consultas resumen:
#1Devuelve el número total de alumnos existentes.

SELECT COUNT(*) AS total_alumnos
FROM persona
WHERE persona.tipo = 'alumno';

#2Calcula cuántos alumnos nacieron en 1999.
SELECT COUNT(*)
FROM persona
WHERE persona.fecha_nacimiento like '1999%';

#3 Calcula cuántos/as profesores/as hay en cada departamento.
SELECT departamento.nombre, COUNT(DISTINCT profesor.id_profesor) AS total_profesores
FROM departamento
...
SELECT grado.nombre, COUNT(DISTINCT asignatura.id) AS total_asignaturas
FROM grado AS grado
LEFT JOIN asignatura AS asignatura
ON grado.id = asignatura.id_grado GROUP BY grado.id
